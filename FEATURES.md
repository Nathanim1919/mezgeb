...
Data integrity, UX polish, and critical fixes.

- [x] **Atomic Transactions** — Transaction insert + balance update in single DB transaction (no more data inconsistency)
- [x] **Currency Formatting** — Thousands separator (1,500,000 ብር instead of 1500000 ብር)
- [x] **Rate Limit Feedback** — Friendly message instead of silent drop when rate limited
- [x] **Transaction Notes** — Optional note/description on every transaction (stored in DB)
- [x] **Input Validation** — Amount cap (10M birr), name length limit (100 chars), price must be > 0

---

## Phase 2 — Sell & Buy Engine (COMPLETED)

Real merchant transaction types with stock management.

### 2.1 Transaction Menu Redesign
- [x] **New transaction menu** — Sell / Buy / Borrow / Loan buttons (replaces old Add Transaction)
- [x] **Sell flow** — Pick product → Quantity → auto-calculate total → Note → Confirm
- [x] **Buy flow** — Pick product → Buy price → Quantity → Note → Confirm
- [x] **New product inline** — "➕ New Product" button during sell/buy if product doesn't exist yet
- [x] **Stock validation on sell** — Can't sell more than available stock
- [x] **Insufficient stock UX** — "Sell All" / "Other Product" / Cancel buttons (not a dead-end error)

### 2.2 Product Management Redesign
- [x] **Product sub-menu** — "Add Product" / "List Products" buttons (replaces old combined view)
- [x] **Stock tracking per product** — Stock quantity stored and displayed
- [x] **Stock on product creation** — Name → Price → Stock quantity flow
- [x] **Rich product list** — Shows price + stock level with low-stock warnings (⚠️ Low / ⛔ 0)
- [x] **Auto stock updates** — Sell decreases stock, Buy increases stock (atomic with transaction)

### 2.3 Borrow & Loan
- [x] **Borrow flow** — "Someone borrowed from me": Customer → Amount → Product (optional) → Note → Confirm
- [x] **Loan flow** — "I borrowed from someone": Person → Amount → Note → Confirm
- [x] **Bidirectional debt** — Customer balance goes positive (they owe you) or negative (you owe them)
- [x] **Customer list shows both** — "owes you 500 ብር" / "you owe 1,000 ብር" / "settled ✓"

### 2.4 Reports Overhaul
- [x] **Sales revenue tracking** — Sum of all sell transactions
- [x] **Expense tracking** — Sum of all buy transactions
- [x] **Profit calculation** — Sales minus Expenses displayed in report
- [x] **Items sold/bought counts** — Quantity totals per period
- [x] **Borrow/Loan in reports** — "Others owe you" / "You borrowed" totals
- [x] **Top sold products** — Ranked by revenue with quantity and amount
- [x] **Smart sections** — Only shows relevant sections (no empty sell/buy/debt sections)

### 2.5 Settings & Data Management
- [x] **Clear All Data** — Delete all transactions, customers, and products
- [x] **Double confirmation** — Warning message + explicit "Yes, delete everything" button
- [x] **Atomic deletion** — All data removed in single DB transaction
- [x] **User preserved** — Language preference kept after clearing data

---

## Phase 3 — Cashflow Visibility

> "Where is my money going?" — Every merchant's first question.
> Give merchants a real-time picture of money in vs. money out.

### 3.1 Expense Tracking & Categories
...
### 3.2 Real-Time Cashflow Dashboard
...
### 3.3 Profit & Loss (Per Transaction)
...
### 4.2 Customer Credit Intelligence
...
### 5.2 Purchase & Restock Management
...
- [ ] Stock adjustment (breakage, loss, personal use)
- [ ] Current stock valuation (total birr sitting in inventory)

---

## Phase 6 — Time-Saving Automation
...
## Phase 7 — Invoicing & Professionalism

> Help merchants look professional and get paid faster.

### 7.1 Digital Receipts
